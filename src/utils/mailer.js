...
async function sendEMail(email,emailType,userId){
    try {
      ...
      const verifyForHTML=`<b>Click <a href='${process.env.DOMAIN}/verifyEmail?${hashedToken}'>here</a></b> to ${emailType=='VERIFY'?'Verify your mail':'Reset Password'}<br> `
      const resetForHTML=`<b>Click <a href='${process.env.DOMAIN}/reset?${hashedToken}'>here</a></b> to ${emailType=='VERIFY'?'Verify your mail':'Reset Password'}<br> `
